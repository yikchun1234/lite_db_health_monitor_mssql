...
def add_server():
    try:
        ...
        server = ServerConfig(alias=alias, host=new_server.get("host", ""), port=new_server.get("port", ""), type=new_server.get("type", "sqlserver"), user=new_server.get("user", ""), password=encrypt_password(new_server.get("password", "")))
        ...
        with engine.connect() as conn:
            ...
            WITH IOLatency AS (
                SELECT DB_NAME(vfs.database_id) AS DatabaseName, 
                       ...
                       SUM(CAST(vfs.io_stall_read_ms AS BIGINT)) AS TotalReadStall,
                       SUM(CAST(vfs.num_of_reads AS BIGINT)) AS TotalReads,
                       SUM(CAST(vfs.io_stall_write_ms AS BIGINT)) AS TotalWriteStall,
                       SUM(CAST(vfs.num_of_writes AS BIGINT)) AS TotalWrites
            ...
            SELECT TOP 10 DatabaseName, Drive, 
                   CASE WHEN TotalReads = 0 THEN 0 ELSE (TotalReadStall / TotalReads) END AS ReadLatency_ms, 
                   CASE WHEN TotalWrites = 0 THEN 0 ELSE (TotalWriteStall / TotalWrites) END AS WriteLatency_ms 
            FROM IOLatency 
            ORDER BY ((CASE WHEN TotalReads = 0 THEN 0 ELSE (TotalReadStall / TotalReads) END) + 
